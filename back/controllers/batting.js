...
exports.getMyBattings = asyncHandler(async (req, res, next) => {
  const collectBattings = await Batting.find(
    { user: req.user._id },
    { battingResult: 'Collect' }
  );
  const wrongBattings = await Batting.find(
    { user: req.user._id },
    { battingResult: 'Wrong' }
  );
  const notFinishedBattings = await Batting.find(
    { user: req.user._id },
    { battingResult: 'Not Finished' }
  );
});
