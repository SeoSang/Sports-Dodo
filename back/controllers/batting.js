...
exports.createBatting = asyncHandler(async (req, res, next) => {
  ...
  const exBatting = await Batting.findOne({
    user,
    match,
  });
  if (exBatting) {
    return next(
      new ErrorResponse(`이미 해당 경기에 베팅한 기록이 있습니다!`, 422)
    );
  }
  ...
  if (chooseHomeAwayDraw == 'Home') {
    await Match.findByIdAndUpdate(
      req.params.id,
      { $inc: { homeBattingNumber: +1, homeBattingPoint: +battingPoint } },
      { new: true }
    );
  } else if (chooseHomeAwayDraw == 'Away') {
    await Match.findByIdAndUpdate(
      req.params.id,
      { $inc: { awayBattingNumber: +1, awayBattingPoint: +battingPoint } },
      { new: true }
    );
  } else {
    await Match.findByIdAndUpdate(
      req.params.id,
      { $inc: { drawBattingNumber: +1, drawBattingPoint: +battingPoint } },
      { new: true }
    );
  }
