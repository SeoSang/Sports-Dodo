...
function reservePutResultToMatch() {
	...
	console.log('reserved calculating result to battings and users everyday 00:02:00');
}
...
async function calculateBattingResultOfEachMatch(matchId, matchResult) {
	...
	for (let j = 0; j < battings.length; j++) {
		...
		let checkBattingChanged = await Batting.findByIdAndUpdate(battings[j]._id, { matchResult: matchResult })
		...
		// plus 포인트 계산.
		if (battings[j].chooseHomeAwayDraw == battings[j].matchResult) {   // batting.chooseHAD == batting.matchResult
			if (matchResult = 'Home') {
				// console.log('2222222222222')
				// console.log(battings[j].battingPoint)
				// console.log(totalPoints.homeTotalPoint)
				// console.log(totalPoints.allTotalPoint)
				plusPoint = ((battings[j].battingPoint / totalPoints.homeTotalPoint) * totalPoints.allTotalPoint);  // if matchResult = home => homeTotalPoint  
			} else if (matchResult = 'Away') {
				plusPoint = (battings[j].battingPoint / totalPoints.awayTotalPoint) * totalPoints.allTotalPoint;
			} else {
				plusPoint = (battings[j].battingPoint / totalPoints.drawTotalPoint) * totalPoints.allTotalPoint;
			}
			...
			// 각 베팅의 결과 입력.
			await Batting.findByIdAndUpdate(battings[j]._id, { battingResult: 'Collect' }, { new: true })
